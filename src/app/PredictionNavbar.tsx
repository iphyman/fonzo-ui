...
import { useHistoryDialog } from "@app/components/HistoryDialog";
import { useAccount } from "wagmi";

const MarketSelector = () => {
  const [open, setOpen] = useState<boolean>(false);
  const { allMarkets, selectMarket, marketId } = useMarket();
  const market = allMarkets?.find((m) => m.feedId === marketId);
  const { data } = useFlareOracle();

  return (
    <Popover.Root
      positioning={{ sameWidth: true, offset: { crossAxis: 0, mainAxis: 0 } }}
      open={open}
      onOpenChange={(e) => setOpen(e.open)}
    >
      <HStack gap={0}>
        <Avatar.Root size="2xl" zIndex={1}>
        ...
        <Popover.Trigger asChild>
          ...
          >
            <HStack justifyContent="space-between" w="full">
              <Text fontWeight={900} fontSize="1.5rem" color="purple.500">
                {market?.name}
              </Text>
              <HStack>
                {data && (
                  <Text fontSize="16px" fontWeight={600}>
                    <CountUp
                      ...
                      delay={0}
                        ...
                        formatUnits(BigInt(data.price), data.decimals)
                      )}
                      prefix="$"
                      decimals={4}
                      duration={1}
            ...
            <Popover.Body paddingX={0}>
              {allMarkets?.map((market) => (
                <Button
                  ...
                  key={market.feedId}
                  disabled={market.feedId === marketId}
                  onClick={() => {
                    selectMarket(market.feedId);
                    setOpen(false);
                  }}
                >
                  ...
                  <Text fontWeight={700} letterSpacing="wider" color="fg.muted">
                    {market.name}
                  </Text>
                </Button>
              ))}
...
const PredictionTimer = () => {
  ...
  const round = data?.rounds.find(
    (m) => m.roundId === BigInt(data.currentRoundId)
  );

  const { countdown } = useCountDown(round?.lockTime);

  return (
    <Box position="relative">
      <HStack padding="4px 54px 4px 8px" bg="bg" borderRadius="1rem">
        <Text fontWeight={700} fontSize="20px" color="purple.500">
...
export function PredictionNavbar() {
  const { address } = useAccount();
  const [, setOpen] = useHistoryDialog();
